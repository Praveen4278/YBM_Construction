...
const FALLBACK_PATH = path.join(FALLBACK_DIR, 'db_fallback.json');
...
const initializeLocalDB = () => {
  ...
  if (!fs.existsSync(FALLBACK_PATH)) {
    fs.writeFileSync(FALLBACK_PATH, JSON.stringify(initialData, null, 2), 'utf-8');
...
export const readLocalDB = (): typeof initialData => {
  try {
    initializeLocalDB();
...
export const writeLocalDB = (data: typeof initialData) => {
  try {
    if (!fs.existsSync(FALLBACK_DIR)) {
      fs.mkdirSync(FALLBACK_DIR, { recursive: true });
    }
